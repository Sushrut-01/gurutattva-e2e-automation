...
Send Test Execution Report Email
    ...
    Log To Console    📧 Preparing to send test execution report email...
    Log To Console    📧 Email Config - Server: ${SMTP_SERVER}:${SMTP_PORT}, From: ${EMAIL_FROM}, To: ${EMAIL_TO}
    ...
    Log To Console    📧 Email Subject: ${email_subject}
    ...
    TRY
        Log To Console    📧 Attempting to send email with Python...
        IF    '${zip_file_path}' != '${EMPTY}' and '${zip_file_path}' != 'None'
            Log To Console    📧 Sending email with ZIP attachment: ${zip_file_path}
            Send Email Using Python    ${email_subject}    ${email_body}    ${zip_file_path}
        ELSE
            Log To Console    📧 Sending email without ZIP attachment
            Send Email Using Python    ${email_subject}    ${email_body}    ${EMPTY}
        END
        Log To Console    ✅ Test execution report email sent successfully
    EXCEPT    AS    ${error}
        Log To Console    ❌ Failed to send email: ${error}
        Log To Console    ⚠️ Continuing test execution without email notification
    END
...
Generate Test Summary Email Body
    ...
    ${status_icon}=    Set Variable If    '${test_status}' == 'PASS'    ✅    ❌
...
Create Report Zip File
    ...
    Log To Console    📦 Creating ZIP file: ${zip_file_name}
    ...
    Log To Console    📦 File existence check - Report: ${report_exists}, Log: ${log_exists}, Output: ${output_exists}
    
    # Try simple ZIP creation by copying files first to avoid file lock issues
    TRY
        Log To Console    📦 Attempting ZIP creation with file copying...
        ...
        Log To Console    📦 Batch execution result: ${result}
    ...
    EXCEPT    AS    ${batch_error}
        Log To Console    ❌ Batch ZIP creation failed: ${batch_error}
        
        # Fallback: Try Python directly
        TRY
            Log To Console    📦 Trying Python ZIP creation as fallback...
            ...
            Log To Console    📦 Python ZIP result: ${result}
        EXCEPT    AS    ${python_error}
            Log To Console    ❌ Python ZIP creation also failed: ${python_error}
        END
    END
    ...
    IF    ${zip_exists} == True
        Log To Console    ✅ ZIP file created successfully: ${zip_file_name}
        RETURN    ${zip_file_path}
    ELSE
        Log To Console    ❌ Failed to create ZIP file
        RETURN    ${EMPTY}
    END
...
Send Email Using Python
    ...
    Log To Console    📧 Email sending result: ${result}
...
Execute Python One Liner
    ...
    IF    ${rc} == 0
        Log To Console    📧 Python execution successful with 'py'
        Log To Console    📧 Python output: ${out}
    ...
    IF    ${rc} == 0
        Log To Console    📧 Python execution successful with 'python'
        Log To Console    📧 Python output: ${out}
        # Extract the result from the output
        ${lines}=    Split String    ${out}    separator=\n
        ${result}=    Set Variable    ${EMPTY}
    ...
    Log To Console    📧 Python execution result: RC=${rc}, Output=${out}
    RETURN    ${out}
...
Configure Email Settings
    ...
    Log To Console    ✅ Email settings configured:
    Log To Console    📧 SMTP Server: ${SMTP_SERVER}:${SMTP_PORT}
    Log To Console    📧 Username: ${SMTP_USERNAME}
    Log To Console    📧 Recipients: ${EMAIL_TO}
